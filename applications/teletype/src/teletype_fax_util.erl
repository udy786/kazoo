...
maybe_fetch_attachments(DataJObj, FaxJObj, Macros, 'false') ->
    ...
    case kz_datamgr:fetch_attachment(Db, {kzd_fax:type(), FaxId}, AttachmentName) of
        {'ok', Bin} ->
            ...
            maybe_convert_attachment(DataJObj, FaxId, Macros, ContentType, Bin);
        {'error', _E} ->
            lager:debug("failed to fetch attachment ~s: ~p", [AttachmentName, _E]),
            []
    end.

-spec maybe_convert_attachment(kz_json:object(), kz_term:ne_binary(), kz_term:proplist(), kz_term:api_binary(), binary()) -> attachments().
maybe_convert_attachment(DataJObj, FaxId, Macros, FromFormat, Bin) ->
